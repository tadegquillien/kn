...
import { knowledgeFirst, color_palette, ballsList } from './gameParameters';
...
function Test({ containsAlpha, urnDimensions, nalpha, nbeta, nalphaActual, nbetaActual, nwhiteActual,
    ...
    const [knowledgeResponse, setKnowledgeResponse] = useState(0);
    ...
    const handleNext = () => {
        // record data
        Data.knowledgeResponses.push(knowledgeResponse);
        Data.beliefResponses.push(beliefResponse);
    ...
    //the likert scale for knowledge
    const likertOptions = {
        question: "",
        responses: likertChoicesTest,
        //keeps track of the last response by the participant
        onChange: val => {
            setKnowledgeResponse(val.value);
            setShowNextButton(1);
        },
        id: 'knowledgeQuestion',
    };

    //the likert scale for belief
    const likertOptionsBelief = {
        question: "",
        responses: likertChoicesTest,
        //keeps track of the last response by the participant
        onChange: val => {
    ...
    const questionPrompt = <span style={{ textAlign: "center", visibility: showQuestion ? " visible" : "hidden" }}>
        <h3>Please tell us how much you agree with the following statements. After seeing the test results...</h3>
    </span>
    //the question 
    const knowledgeQuestion = <span
        ...
        <h3 >...The farmer knows that the tree is {containsAlpha ? "" : "NOT"} sick</h3>
        <div //</span>style={{ marginLeft: "20vw", marginRight: "20vw" }}
        ><Likert {...likertOptions} className="lik" /></div>
    </span>

    const beliefQuestion = <span
        style={{ textAlign: "center", visibility: showQuestion ? " visible" : "hidden" }}
        className="beliefQuestion">

        {/* <h3 >Please tell us how much you agree with the following statement:</h3> */}
        <h3 >...The farmer believes that the tree is {containsAlpha ? "" : "NOT"} sick</h3>
        <div //</span>style={{ marginLeft: "20vw", marginRight: "20vw" }}
        ><Likert {...likertOptionsBelief} /></div>
    </span>
    ...
    const nextPageButton = <button style={{
        ...buttonStyle, visibility: ((beliefResponse > 0) & (knowledgeResponse > 0)) ?
            "visible" : "hidden"
    ...
    // order the questions
    const questions = knowledgeFirst ? <div class="horizontal">{knowledgeQuestion}{beliefQuestion}</div> :
        <div class="horizontal">{beliefQuestion}{knowledgeQuestion}</div>;
